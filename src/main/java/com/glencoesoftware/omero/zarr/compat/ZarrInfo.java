 ...
 * ngff versions. To access the zarr metadata/array use getZarrPath().
 */
public class ZarrInfo {
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(ZarrInfo.class);
    ...
    /**
     * Create a new ZarrInfo.
     *
     * @param location the location of the zarr
     * @throws IOException if the zarr can't be read
     */
    public ZarrInfo(String location) throws IOException {
        this.location = location.endsWith("/") ? location.substring(0, location.length() - 1)
            : location;
        checkProperties();
    }

    /**
     * Tries to check some properties of the zarr path, if it's remote or local, and the zarr and
     * ngff versions.
     *
     * @throws IOException If the zarr can't be read
     */
    private void checkProperties() throws IOException {
        ...
        try {
            ...
            try {
                ngffVersion = new ComparableVersion(
                    ((Map<String, Object>) md.attributes.get("ome")).get("version").toString());
            } catch (Exception e) {
        ...
        try {
            ...
            try {
                ...
                ngffVersion = new ComparableVersion(
                    ((Map<String, Object>) tmp.get(0)).get("version").toString());
            } catch (Exception e) {
                log.debug("Failed to get ngff version from zarr, set to 0.4");
                ngffVersion = new ComparableVersion("0.4");
                // if it's zarr v2 then we can actually safely assume it's ngff v0.4
            }
            return;
        } catch (Exception e) {
    ...
    /**
     * Converts an NGFF root string to a path, initializing a {@link FileSystem} if required For
     * zarr version 2, resp NGFF 0.4
     *
    ...
    /**
     * Return a store handle. For zarr version 3, resp NGFF > 0.5
     *
    ...
    private StoreHandle asStoreHandle() {
        try {
            URI uri = new URI(location);
            String scheme = uri.getScheme() != null ? uri.getScheme().toLowerCase() : "file";
            String locationNoParams = location.replaceAll("/?\\?.+", "");
            String store = locationNoParams.substring(0, location.lastIndexOf("/"));
            String zarr = locationNoParams.substring(location.lastIndexOf("/") + 1);
    ...
    public String toString() {
        return "ZarrInfo{" + "location='" + location + '\'' + ", storageType=" + storageType
            + ", zarrVersion=" + zarrVersion + ", ngffVersion=" + ngffVersion + '}';
    }
