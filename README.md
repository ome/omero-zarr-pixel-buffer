...
[OME CI](https://merge-ci.openmicroscopy.org/jenkins/job/OMERO-zarr-pixel-buffer-build/)
...
-   either by adding the JAR file and all its dependencies under the `lib/server` directory
    of OMERO.server and restarting the server (see [install.sh](install.sh)),
...
-   the Zarr dataset must be conformant with the
    [Zarr specification](https://zarr-specs.readthedocs.io/en/latest/specs.html) version 2 or newer.
    and the [NGFF specification](https://ngff.openmicroscopy.org/) version 0.4 or newer.
...
-   `lsid` must be set to the path of the Zarr group describing the
    ...
    - or the URI specifying the location of the multiscales group if the Zarr is
      stored on AWS S3 or S3 compatible storage. It must be percent-encoded as defined in
      ...
      - or as http(s) URL, e.g. `https://uk1s3.embassy.ebi.ac.uk/idr/zarr/v0.4/idr0079A/idr0079_images.zarr/0
...
Clone the repository:
    ...
    git clone https://github.com/ome/omero-zarr-pixel-buffer
